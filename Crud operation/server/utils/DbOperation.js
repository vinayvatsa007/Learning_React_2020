...
const { response } = require("../routers");
...
// dbCon = mysql.createConnection(config1);
...
const find = async (tableName, filters) => {
  try {
    ...
    // although we passing 2nd param but its useless because query already framed through template string, but result will come if we pass the callback function as 3rd param
    const [data, fields] = await dbCon.promise().query(query, tableName);
    sortedData = getSortedData(data, name, order);
    console.log("dboperations.js-> find-> sortedData: ", sortedData);
    paginatedData = getPaginatedData(
      sortedData,
      offset,
      parseInt(offset) + parseInt(size) - 1
    );
    return new Responses.ListResponse(paginatedData);
  } catch (error) {
    throw new Responses.ErrorResponse(error);
    // console.log('find->catch error thrown',error);
    // throw error;
...
const insertRecord = async (tableName, objRecord) => {
  // console.log("tableName, objRecord", tableName, objRecord);
  try {
...
//     });
// };
const totalCount = async (tableName) => {
  try {
    const query = `SELECT count(*) as totalCount FROM ${tableName}`;

    const [data, fields] = await dbCon.promise().query(query);

    const { totalCount } = data[0];
    return new Responses.ObjectResponse(totalCount);
  } catch (error) {
    return new Responses.ErrorResponse(error);
  }
};

const getResultsByQuery = async (queryStatement) => {
  try {
    const [data, fields] = await dbCon.promise().query(queryStatement);
    return data;
  } catch (error) {
    return new Responses.ErrorResponse(error);
  }
