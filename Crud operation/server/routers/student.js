...
const { convertImageBufferToBase64 } = require("../utils/index");
...
router.get(
  ...
  asyncHandler(async (req, resp) => {
    try {
      let data = await find("student", req.query);
      // console.log("find->data->", data);
      data.results = data.results.map((record) => {
        const base64Photo = convertImageBufferToBase64(record.photo);
        console.log("find->data->record----:", base64Photo);
        return { ...record, photo: base64Photo };
      });
      // for (let rec in data.results) {
      //   console.log("find->data->rec----:", data.results[rec].photo);
      // }
      // console.log("find->data->", data);
      resp.send(data);
    } catch (error) {
