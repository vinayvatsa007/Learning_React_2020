...
class BaseService {
  ...
  find = async (filters) => {
    try {
      ...
      // route=> assignments/
      // console.log("find method filters ---", filters);
  ...
  totalCount = async () => {
    try {
      const serviceURL = this.getUrl(`totalCount`);
      //Node Services hosting URL ---- http://localhost:3010/
      // route=> assignments/totalCount
      console.log("totalCount method serviceURL ---", serviceURL);
      const resp = await request.get(serviceURL);
      // console.log("totalCount method results ---", resp);
      return resp.body;
    } catch (error) {
      console.log("Error_from_BaseService.js", error);
      throw error;
